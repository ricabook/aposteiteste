-- Corrigir função get_simple_odds para não gerar P&L positivo quando há apenas um apostador
...
BEGIN
  ...
  FROM bets
  WHERE poll_id = _poll_id AND option_chosen = _option AND amount > 0;
  
  -- Get total volume for other options
